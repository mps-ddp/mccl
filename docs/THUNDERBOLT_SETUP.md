...
Pick **one machine as rank 0**. Everyone uses **that machine’s** Thunderbolt IP as `MASTER_ADDR`.
...
export MCCL_LISTEN_ADDR=169.254.238.250   # This Mac’s own IP on the TB link
...
```bash
export MASTER_ADDR=169.254.238.250   # Still rank-0’s IP
export MASTER_PORT=29500
export MCCL_LISTEN_ADDR=169.254.238.251   # *This* Mac’s TB IP (not A’s)
...
Replace the `169.254.*` addresses with what `ifconfig` shows on **each** Mac.
...
2. **MCCL** opens **separate TCP sockets** (per peer) for collective traffic; ranks publish listen addresses (see `MCCL_LISTEN_ADDR` in [MULTINODE.md](MULTINODE.md)).
...
- **Connection refused:** Blocked `MASTER_PORT` or MCCL listen ports; fix firewall or bind addresses.
